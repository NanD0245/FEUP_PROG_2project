...
/*=================|INITIAL FUNCTIONS|=================*/
//Functions that only are used in the beginning of the game
...
int Scrabble_Junior::tiles_perPlayer(int number_players)			//Depending on the size of the initial pool, returns the number of tiles that should be given per player in the beggining of the game
{
	if ((pool.getSize() / number_players) >= HAND_MAXSIZE)			//If the pool is able to give 7 or more tiles per player, only will be given 7 tiles, because it's the players hands maximum size
		return HAND_MAXSIZE;
	return (pool.getSize() / number_players);						//Else, returns the maximum number of tiles that can be given equally at all the players
}
...
void Scrabble_Junior::addPlayer(Player p)
{
	players.push_back(p);
}
...
Player Scrabble_Junior::nextTurn()								//Decides who is gonna play next
{
	Player p;
...
void Scrabble_Junior::showScore(Player &p) const										//shows on the screen the score of all the players in the order of who plays first
{
...
bool Scrabble_Junior::validHand(const Player &p, Board &board) const			//Verifies if the player has some tile available to play
{
	...
	for (auto elem : player_hand)
	{
		if (board.isValid(elem))
			return true;														//If the player has some tile to play, return true
	}
	return false;
}
...
void Scrabble_Junior::refreshHand(Player &p)										//The player get 1 letter from the pool
{
	char l;
	l = pool.giveTile();
	p.addtoHand(l);
...
void Scrabble_Junior::updatePlayer(Player p)										//Update the player in the list
{
...
{
	Player p;																		//The constructor of Player puts score = 0, this object is used for comparation
	vector<Player> v_players;
	for (auto player : players)
	{
		if (player.getScore() > p.getScore())										//If the score of the list element is biggest than p, the vector with the winners is updated
		{
			p = player;
			v_players.clear();
			v_players.push_back(player);
		}
		else if (player.getScore() == p.getScore())									//else if is equal, add the player to the list
		{
			v_players.push_back(player);
		}
	}
