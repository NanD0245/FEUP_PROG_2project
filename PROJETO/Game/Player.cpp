...
bool operator==(const Player& left, const Player& right) //operator used in the comparation of players
{
...
void Player::addHand(vector<char> x)		//Function used to add more than one tile to the player hand
{
...
void Player::addtoHand(char x)				//Function used to add a tile to the player hand
{
	_hand.insert(x);
}
...
void Player::clearHand()					//Funtion used to remove all the tiles of the player hand
{
	_hand.clear();
}
...
bool Player::isValid(char x) const			//Verifies if the player has the tile that wants to play
{
	if (_hand.find(x) == _hand.end())
		return false;
	return true;							//If the player has the tile, the function return true
}

void Player::tileChoosed(char x)			//Function used to remove a tile that already has been played
{
	auto it = _hand.find(x);
	_hand.erase(it);
}

void Player::showHand()	const				//Shows in the screen the hand of the player
{
	auto it = _hand.begin();
...
void Player::resetPlayer()			//clear all the information off the player
{
	_score = 0;
	_name = "";
	_hand.clear();
