...
void fileName(string& file)
{
	...
	do
	{
		...
		else if (file.find(".txt") > file.length())						//if the given name don't have the extension ".txt", the program adds it at the end of the string
		{
			file += ".txt";
		}
		str_in.open(file);
		if (!str_in.is_open())
		{
			validInput = false;											//If the file name given is invalid, repeat all again
		}
		if (!validInput)
		{
			setColor(ERROR_COLOR);
	...
	do
	{
		...
		do
		{
			...
			if (cin.fail())
			{
				...
				if (play)
				{
					play = false;										//When the input fails, the program get back and ask again to the user what to  do
					str = " ";
				}
				else
				{
		...
		if (play)														//play = false if the player wants to go to the pool, player = true if the player wants to play a tile
		{
			do
			{
				...
				{
					if (str[0] < LETTER_A || str[0] > char(LETTER_A + lines))
					{
						invalidInput(validInput);
						cout << endl;
					}
					else if (str[1] < LETTER_a || str[1] > char(LETTER_a + column))
					{
						invalidInput(validInput);
						cout << endl;
					}
					else
					{
						...
						else
						{
							invalidInput(validInput);
							cout << endl;
							again = true;								//validInput continues true because the player may change the letter that wants to play
						}
					}
				}
			} while (!validInput);
...
void play()
{
	...
	do
	{
		...
		do
		{
			...
			getline(cin, str, '\n');
			toUpper(str);
			if (cin.fail())												//If the player use something that makes the input fail, backs to the menu
			{
				cin.clear();
				back = true;
			}
		...
		do
		{
			...
			else if (validInput && numberTiles < MINIM_PER_PLAYER)		//If don't occured an error during the input phase
			{
				setColor(ERROR_COLOR);
				cout << "To mutch players for the number of letters of the board. ";
				validInput = false;
				setColor(TEXT_COLOR);
			}
		} while (!validInput);
		...
		do
		{
			...
			played = false;												//played = true if the player play some tile
			...
			do
			{
				...
				{
					setColor(ERROR_COLOR);
					if (!validHand && played)							//if the player hasn't no more tiles no play, but had already played 1 tile 
					{
						showHand = false;
						cout << "You already played 1 tile and there is no more to play... You should pass. " << endl;
					}
					...
					else
					{
						...
						if (str != " ")									//If str == " ", the player wants to go back in the decision
						{
							...
							clrscr();
							game.showScore(player);
							board.showBoard();							//Show in the board, the letter played in another color
				...
				{
					if (verify_hands)									//The hands of the others players is already on the screen
					{
						setColor(ERROR_COLOR);
						showHand = false;
						cout << "You already saw your hand in this round..." << endl;
						setColor(TEXT_COLOR);
					}
					else
					{
				...
				{
					setColor(ERROR_COLOR);
					if (validHand)										//If the player has some tile available to play
					{
						showHand = false;
					...
					{
						showHand = false;
						cout << "The pool is empty and you don't have any tile too play... You should pass. " << endl;
					}
					else if (played)
					{
						showHand = false;
						cout << "You already played 1 tile and theres no more to play, so you can't go to the pool... " << endl;
					}
					else
					{
						showHand = true;
			...
			if (played)														//If any tile has been played in this turn, it will be replaced if possible
			{
				...
				{															//Or the number of tiles replaced is equal to the number of the tiles played
		...
		game.showScore(player);
		board.showBoard();
		winners = game.endGame();											//Winners has the players the highest score in the end of the game
...
void rules()
{
	...
	cout << endl
		...
		<< "\n(Press any key to continue...)" << endl; (void)_getch(); //leave this menu only when the user press a key
	clrscr();
}
