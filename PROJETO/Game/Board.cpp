...
void Board::addLetters(pair<int, int> position, const string word, vector<char>& pool, const pair<int, int> sum) //Function used to add letters to the pool and to the map Letters
{
	for (size_t i = 0; i < word.length(); i++)
	{
		...
		{
			letters[position] = word[i];							//If the position isn't in the map of the letters, it's a new letter so it needs to be added at the map and at the pool
...
void Board::setBoard(const string file)
{
	...
	char direction, x_ignore;						//The char 'x' is the char between th number of the lines and the number of columns (it must be ignored)
	...
	*/
	...
	while (getline(str_in, line, '\n'))													//while is possible to read the lines of the given file
	{
		...
		position = newPair(temp_position[0], temp_position[1]);							//turns the temp_position to position: from (char, char) to (int, int)															//The word starts in the 6th position of the line, so 'text' is now the word to use in the board
		toUpper(word);

		if (direction == 'H')															//if the direction of the word is H, it should be added int the map of the horizontal word
		{
		...
		}																				//pair<int, int>(0, 1) becausethe direction is H, so we will need to increment the number of the line
		else
		{
			wordsV[position] = word;													//Otherwise, the word is added in the map of the vertical words
			addLetters(position, word, pool, pair<int, int>(1, 0));						//pair<int, int>(1, 0) because the direction is V, so we wiil need to increment he number of the column
		}
	}
	str_in.close();
	initialLetters = pool;
...
vector<char> Board::getPool() const		//Returns a vector with the all the letters used in the board
{
...
{
	for (auto elem : wordsH)							//Verify if there are any word that the first letter is equal to the chosen letter
	{
		if (elem.second[0] == l)
			return true;
	}
...
*@param position is the position of the letter that was been played, the position of the first letter of the word
...
int Board::modifyHVMap(map<pair<int, int>, string>& modify, pair<int, int> position, const pair<int, int> sum)
{
	...
	modify.erase(position);									//remove the position because the position that must be assign to the new word the position of the next letter available in the word
	if (word.length() != 0)									//If there are more letters available in the word, the map must be updated
	{
		...
		return 0;											//When the position was found, update the map and return 0, because the tile played wasn't the last letter of the word	
	}
	return 1;												//If there are no more letters available in the word the player will score 1
}


/*Function use to verify if the player played a letter in the mid of the word (because of the crossed words)
*
...
*@param position is the position of the letter that was been played
...
int Board::crossedWords(map<pair<int, int>, string>& modify, pair<int, int> position, const pair<int, int> sum, const int lim)
{
	...
	{
		verification.second = position.second;							//	 in this case it's necessary to find if there are any word in the same column with a letter in the position played
	}
	else																//If sum = (0,1), modify = wordsH because all the words are in the same line
	{
		verification.first = position.first;							//	 in this case it's necessary to find if there are any word in the same line with a letter in the position played
	}
	...
	for (int i = 0; i < lim; i++)										//Verify if there is a word in the given line/column
	{
		if (modify.find(verification) != modify.end())
		{
			...
			{
				if (position == verification)							//If there are a letter in the given position it belongs to 2 words (V and H)
				{
...
{
	int score = 0;
	if (wordsH.find(position) != wordsH.end())														//If there are a word in wordsH with the first letter in the given position
	{
		score += crossedWords(wordsV, position, pair<int, int>(1, 0), lines);
		score += modifyHVMap(wordsH, position, pair<int, int>(0, 1));
	}
	else if (wordsV.find(position) != wordsV.end())													//If there are a word in wordsV with the first letter in the given position
	{
		score += crossedWords(wordsH, position, pair<int, int>(0, 1), columns);
		score += modifyHVMap(wordsV, position, pair<int, int>(1, 0));
	}
	colouredLetters[position] = letters[position];													//The letter played should now appear in the board with another color
...
void Board::showBoard()									//This function shows on the screen the game board
{
	...
	for (int i = -1; i < lines; i++)					//This loop updates the number of the line that is going to be shown
	{
		...
		for (int j = -1; j < columns; j++)				//This loop updates the number of the column that is going to be shown
		{
			...
			else if (i == -1 && j != -1)				//first line of the table, with the with the identification of the columns
			{
				setColor(EDGES);
				cout << ' ' << char(j + LETTER_a) << ' ';
			}
			else if (j == -1 && i != -1)				//first column of the table, with the identification of the lines
			{
				setColor(EDGES);
				cout << "\t\t\t " << char(LETTER_A + i) << ' ';
			}
			else
			{
				setColor(GAMEBOARD);
...
bool Board::endGame() const			//Function that determines if the game should end
{
