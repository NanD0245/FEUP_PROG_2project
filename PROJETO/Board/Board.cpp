...
void Board::getDimension() {		//Asks the user for the size of the board
	...
	do {
		...
		Helpers::cleanInput();
		for (size_t i = 0; i < str_width.size(); i++)
			if (int(str_width[i]) < 48 || int(str_width[i]) > 57) {
				check = false;
				break;
			}
		if (check)
			width = stoi(str_width);
		if (check == false || 0 >= width || width > MAX_SIZE_LIMIT)
			cout << RED << "Invalid input. Input a number between 0 and 20" << NO_COLOR << endl;
	} while (check == false || 0 >= width || width > MAX_SIZE_LIMIT);
	cout << "Board_letters's height (max = "<< MAX_SIZE_LIMIT << "):" << endl;
	do {
		check = true;
		getline(cin,str_height);
		Helpers::cleanInput();
		for (size_t i = 0; i < str_height.size(); i++)
			if (int(str_height[i]) < 48 || int(str_height[i]) > 57) {
				check = false;
				break;
			}
		if (check)
			height = stoi(str_height);
		if (check == false || 0 >= height || height > MAX_SIZE_LIMIT)
			cout << RED << "Invalid input. Input a number between 0 and 20" << NO_COLOR << endl;
	} while (check == false || 0 >= height || height > MAX_SIZE_LIMIT);
}
...
string Board::getWord() {		//Asks the user for a word and verifies if it belongs to the dictionary 
	...
	do {
		...
		Helpers::cleanInput();
		if (str_word != "STOP" && str_word != "UNDO" && str_word != "RESET" && words.checkWord(str_word) == false) 
			cout << RED << "Input invalid. Please input another word in lowercases or one of 'STOP', 'UNDO' or 'RESET'" << NO_COLOR << endl;
	} while (words.checkWord(str_word) == false && str_word != "STOP" && str_word != "UNDO" && str_word != "RESET");
	return str_word;
}
	...
	int i, j;
			...
			for (j = 1; j <= width; j++) 
				...
				if (int(board_letters[i][j]) != SPACE_ASCII_NUM && (int(board_letters[i][j - 1]) == SPACE_ASCII_NUM || j == 1) && j<width && int(board_letters[i][j + 1]) != SPACE_ASCII_NUM) {	//first letter of one word
					...
					if (j == width && board_letters[i][j] != SPACE_ASCII_NUM) 	//if the last letter is on the limit of the board
						board_word.appendLetterWord(board_letters[i][j]);
					if (words.checkWord(board_word.getWord())) {	//if the word is in the dictionary add to the list of the words
		...
		for (j = 1; j <= width; j++) 	//vertical
			for (i = 1; i <= height; i++) 
				...
				else if (int(board_letters[i][j]) != SPACE_ASCII_NUM && board_word.getWord() != "") 	//middle letters of one words
					board_word.appendLetterWord(board_letters[i][j]);
		return true;
}
...
void Board::showBoardLetters() {																					//Clears the terminal and prints the board
	int i, j;
	cout << "\033[2J\033[H";	//cleans the terminal
	for (i = 0; i <= height; i++) {
		for (j = 0; j <= width; j++) 
...
void Board::showMessage() {	//Prints the message, yellow if it is a warning or red if it is a error
...
bool Board::addNewWord() {		//Function that add a new word to the board if there aren't errors
...
void Board::newWords() {		//This function does a warning about the words that were added or modified to the board
	size_t i, j;
	bool check;
	...
	for (i = 0; i < all_generated_words.size(); i++) {
		...
		for (j = 0; j < all_generated_words_copy.size(); j++) {
			if (all_generated_words[i].getWord() == all_generated_words_copy[j].getWord() && all_generated_words[i].getOrientation() == all_generated_words_copy[j].getOrientation() 
				&& all_generated_words[i].getCoordinates() == all_generated_words_copy[j].getCoordinates()) { 		//if the objecto word is in the original and in the copy has not been modified
				check = true;
				break;
			}
		}
